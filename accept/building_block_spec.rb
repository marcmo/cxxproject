...
describe Cxxproject::BuildingBlock do
  ...
  it 'should collect the dependencies for a single lib with one dep' do
    lib1 = Cxxproject::StaticLibrary.new('1')
    lib2 = Cxxproject::StaticLibrary.new('2').set_dependencies(['1'])
    lib2.collect_dependencies().should == [lib2, lib1]
  end

  it 'should build the right dependency-chain' do
    lib1 = Cxxproject::StaticLibrary.new('1').set_dependencies(['2'])
    lib2 = Cxxproject::StaticLibrary.new('2')
    lib3 = Cxxproject::StaticLibrary.new('3').set_dependencies(['2', '1'])
  ...
  it 'should build the right dependency-chain for custom blocks' do
    lib1 = Cxxproject::StaticLibrary.new('1')
    lib2 = Cxxproject::StaticLibrary.new('2').set_dependencies(['1'])
    lib3 = Cxxproject::StaticLibrary.new('3').set_dependencies(['1', '2'])
    lib4 = Cxxproject::StaticLibrary.new('4').set_dependencies(['1', '2', '3'])
    lib4.collect_dependencies().should == [lib4, lib3, lib2, lib1]
  end

  it 'should generate an error if building block names conflict' do
    expect {
      Cxxproject::StaticLibrary.new('1')
      Cxxproject::StaticLibrary.new('1')
    }.to raise_exception
  end
  ...
  it 'should handle whole archive' do
    ...
    l1 = Cxxproject::StaticLibrary.new('1', true)
    l1.output_dir = 'out'
    l1.complete_init
    l2 = Cxxproject::StaticLibrary.new('2', true)
    l2.output_dir = 'out'
    l2.complete_init
  ...
  it 'should have tags' do
    s1 = Cxxproject::StaticLibrary.new('s1')
    s1.tags = ["a", "b"].to_set
  end
  ...
  it 'should be possible to find building blocks by tag' do
    s1 = Cxxproject::StaticLibrary.new('s1')
    s1.tags = ["a", "b"].to_set
    s2 = Cxxproject::StaticLibrary.new('s2')
    s2.tags = ["b"].to_set
...
=begin
  it 'should have the right output-directory' do
    lib1 = Cxxproject::StaticLibrary.new('lib1').set_sources(['test.cc'])
    lib1.set_project_dir(File.join(Dir.pwd, 'lib1'))

    lib2 = Cxxproject::StaticLibrary.new('lib2').set_sources(['test.cc']).set_output_dir('build2')
  ...
  it 'should raise exception if building block cannot be resolved' do
    expect do
      lib1 = Cxxproject::StaticLibrary.new('1').set_dependencies(['unresolved'])
      cxx = CxxProject2Rake.new([], 'build', compiler)
